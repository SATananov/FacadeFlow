...
const fail = (message) => {
  console.error(`DESKTOP 01C.3F VERIFY FAIL: ${message}`)
  process.exit(1)
}
...
const publisher = read('scripts/publish-desktop01c3f-release.ps1')
...
if (!atLeast(pkg.version, '0.1.5')) fail(`current source version must be >= 0.1.5; found ${pkg.version}`)
...
if (pkg.scripts?.['test:desktop01c3f'] !== 'node scripts/verify-desktop01c3f.mjs') fail('01C.3F verifier command missing')
if (!pkg.scripts?.['test:contract']?.includes('npm run test:desktop01c3f')) fail('01C.3F not registered in full verify')
...
if (!publisher.includes("$expected = '0.1.5'")) fail('publisher does not target 0.1.5')
if (!publisher.includes("$previousTag = 'v0.1.4'")) fail('publisher previous-release guard missing')
if (!publisher.includes("Installed FacadeFlow seed must be 0.1.4")) fail('publisher installed-seed guard missing')
...
console.log('=== DESKTOP 01C.3F VERIFY PASS ===')
console.log(`CURRENT SOURCE / VISIBLE VERSION: ${pkg.version}`)
console.log('INSTALLED TEST SEED: MUST REMAIN 0.1.4 UNTIL REAL 0.1.5 UPDATE TEST')
...
console.log('RELEASE TARGET: v0.1.5 / FacadeFlow-Update-0.1.5.exe')
