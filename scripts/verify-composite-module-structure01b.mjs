...
function panelHarness() {
  ...
  function render() { cursor = 0; tree = Panel() }
  ...
  render()
  return { byId, change, click, elements, text, draft: () => slots[0], remount() { slots = []; render() } }
}
test('panel workflow: two parts, independent sides and dimensions, ZERO_DIVIDER and summary', () => {
  const h = panelHarness()
  assert.equal(h.byId('composite-system').props.value, '')
  assert.ok(h.elements().find((node) => node.type === 'button' && h.text(node) === 'Добави рамкова част').props.disabled)
  h.change('composite-system', 'kmg-prelude-60')
  ...
  h.change('composite-system', 'kmg-prestige-70')
  assert.ok(h.draft().frameParts.every((part) => part.frameProfileCode === null))
  assert.deepEqual(h.draft().frameParts[1].frameSides, sideSelection)
  const options = h.elements(h.byId(`composite-profile-${doorId}`)).filter((node) => node.type === 'option' && node.props.value)
  assert.deepEqual(options.map((node) => node.props.value), ui.compositeFrameCandidates('kmg-prestige-70').map((profile) => profile.code))
  h.change('composite-from', windowId)
  h.change('composite-to', doorId)
...
test('source contracts: 01A reuse, isolated navigation, no geometry/models/persistence claims', () => {
  ...
  assert.match(app, /id: 'composite-structure', label: 'Структура на модул'/)
  assert.match(app, /headerSection === 'composite-structure' \? \(\s*<CompositeModuleStructurePanel \/>/)
  ...
  assert.match(panel, /не се записва/)
...
test('01A authority and existing persistence/schema retain approved checkpoint content', () => {
  const baseline = {
    'src/domain/compositeModuleStructure.ts': '8171ba08ce586fc102486b59fae5735a9498c82c56a1b5b0e4b52ccd2ab95435',
    'src/domain/project/projectModel.ts': '830f6501be72dfe1c85f7b7c69d7b6399662a8027fcfa0477b66dba92093ff76',
