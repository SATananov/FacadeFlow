...
if ($head -ne $origin) { Fail 'HEAD is not identical to origin/master. Commit and push 0.1.5 first.' }
...
if (-not $installedVersion.StartsWith('0.1.4')) { Fail "Installed FacadeFlow seed must be 0.1.4 for the real update test; found $installedVersion." }
...
Write-Host '=== VERIFY SOURCE 0.1.5 ===' -ForegroundColor Cyan
npm run test:desktop-handoff-survival
if ($LASTEXITCODE -ne 0) { Fail 'Windows handoff survival test failed.' }
npm run test:desktop01c3f
if ($LASTEXITCODE -ne 0) { Fail 'DESKTOP 01C.3F verifier failed.' }

Write-Host '=== BUILD UPDATE PACKAGE 0.1.5 ===' -ForegroundColor Cyan
...
Write-Host '=== PACKAGED 0.1.5 SMOKE ===' -ForegroundColor Cyan
& $unpacked --smoke-test
if ($LASTEXITCODE -ne 0) { Fail 'Packaged app smoke failed.' }

$notes = @"
FacadeFlow $version Windows-native updater end-to-end release.
- Installed test seed: 0.1.4
...
- Electron waits for atomic authorization + worker acknowledgement before quitting
- Worker repeats canonical filename, size, SHA-256 and MZ checks before NSIS /S
- Expected flow: 0.1.4 detects 0.1.5, downloads it, user confirms update, FacadeFlow closes, NSIS updates, FacadeFlow relaunches as 0.1.5
- User-data preservation boundary remains unchanged
"@

Write-Host '=== CREATE GITHUB RELEASE v0.1.5 ===' -ForegroundColor Cyan
...
Write-Host ''
Write-Host '=== DESKTOP 01C.3F RELEASE PUBLISHED ===' -ForegroundColor Green
Write-Host "TAG: $tag"
Write-Host "ASSET: $expectedAsset"
Write-Host "BLOCKMAP: $expectedBlockmap"
Write-Host 'SOURCE: origin/master matches HEAD'
Write-Host "INSTALLED LOCAL APP: $installedVersion (KEEP 0.1.4 FOR TEST)"
Write-Host 'NEXT: open installed FacadeFlow 0.1.4 -> Check -> Download -> Update and restart.'
