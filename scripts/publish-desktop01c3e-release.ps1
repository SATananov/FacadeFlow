...
if ($head -ne $origin) { Fail 'HEAD is not identical to origin/master. Commit and push 0.1.4 first.' }
...
if (-not $installedVersion.StartsWith('0.1.3')) { Fail "Installed FacadeFlow seed must be 0.1.3 for the real update test; found $installedVersion." }
...
Write-Host '=== BUILD UPDATE PACKAGE 0.1.4 ===' -ForegroundColor Cyan
...
Write-Host '=== PACKAGED 0.1.4 SMOKE ===' -ForegroundColor Cyan
& $unpacked --smoke-test
if ($LASTEXITCODE -ne 0) { Fail 'Packaged app smoke failed.' }

$notes = @"
FacadeFlow $version READY-ACK updater retest release.

- Installed test seed: 0.1.3
- Manual check for updates: enabled
- In-app download: enabled
- Install + restart: human initiated only
- READY-ACK helper handshake before app quit
- Expected flow: 0.1.3 detects 0.1.4, downloads it, user confirms update, FacadeFlow closes, NSIS updates, FacadeFlow relaunches as 0.1.4
- User-data preservation boundary remains unchanged
"@

Write-Host '=== CREATE GITHUB RELEASE v0.1.4 ===' -ForegroundColor Cyan
...
Write-Host ''
Write-Host '=== DESKTOP 01C.3E RELEASE PUBLISHED ===' -ForegroundColor Green
Write-Host "TAG: $tag"
Write-Host "ASSET: $expectedAsset"
Write-Host "BLOCKMAP: $expectedBlockmap"
Write-Host 'SOURCE: origin/master matches HEAD'
Write-Host "INSTALLED LOCAL APP: $installedVersion (KEEP 0.1.3 FOR TEST)"
Write-Host 'NEXT: open installed FacadeFlow 0.1.3 -> Check -> Download -> Update and restart.'
