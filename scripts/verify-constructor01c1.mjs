...
assert.match(shell, /(?:FIELD TOPOLOGY 01C\.[12]|FRAME INTERIOR 01C\.3|FIELD TOPOLOGY 01C\.3\.2)/)
assert.match(shell, /selectedFieldId/)
assert.match(shell, /fields\.map\(\(field\)/)
assert.match(shell, /constructor-field-surface/)
assert.match(shell, /ПОЛЕ \{field\.sequence\}/)
...
assert.match(shell, /constructor-01c\.[123]/)
